...
# Combined Flask route for all chart data
...
def get_chart_data(chart_type):
    ...
    try:
        ...
        if null_counts.any():
            logger.warning(f"⚠️ [FLASK] Null values found: {null_counts.to_dict()}")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                logger.warning(f"⚠️ [FLASK] Skipping row {idx} due to data conversion error: {e}")
                continue
